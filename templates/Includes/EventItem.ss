<footer itemscope itemtype="http://schema.org/Event">
    ...
    <% if $Date %>
        <time>
            <span itemprop="startDate" datetime="{$Date.Format(yyyy-MM-dd)}<% if $StartTime %>T{$StartTime.Format(HH:mm:ss)}<% end_if %>">
                $Date.Format('d/m/Y') <% if $StartTime %>$StartTime.Nice<% end_if %>
            </span>
            <% if $EndTime %>
                <span itemprop="endDate" datetime="{$Date.Format(yyyy-MM-dd)}T{$EndTime.Format(HH:mm:ss)}">- $EndTime.Nice</span>
            <% end_if %>
        </time>
    <% end_if %>
    </address>
